...
def parse(source):
    ...
    return parse_single_expression(source)


def parse_single_expression(exp):

    if exp[0] == "'":
        # quoted string, syntactic sugar
        return parse_single_expression("(quote %s)" % exp[1:])
    if exp == "()":
        return []
    if exp == "#t":
        return True
    ...
    if re.match("^[^\s\(]+$", exp):
        return exp
    if re.match("^\((.|\n)+\)$", exp):
        return parse_list_expression(exp)

    raise LispError("Not a valid single_expression: %s" % exp)

def parse_list_expression(exp):
    assert (exp[0] == '(')

    end = find_matching_paren(exp, 0)

    # list is expected to extend over whole expression
    if not (end == len(exp) - 1):
        raise LispError('Expected EOF')

    unpacked = exp[1:end]

    expressions = split_exps(unpacked)
    return [parse_single_expression(e) for e in expressions]
